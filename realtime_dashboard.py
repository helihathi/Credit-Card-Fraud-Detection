...
with colg:
    try:
        ...
        gauge.update_layout(height=300, margin=dict(l=10, r=10, t=30, b=10))
...
# 7. Fraud by Device
if "device" in display_df.columns and "fraud_probability" in display_df.columns:
    st.markdown("#### 📱 Fraud by Device")
    try:
        dev = display_df.groupby("device")["fraud_probability"].mean().reset_index()
        fig_dev = px.bar(dev, x="device", y="fraud_probability", title="Average Fraud Probability by Device")
        fig_dev.update_layout(height=300)
        st.plotly_chart(fig_dev, use_container_width=True)
    except Exception as e:
        st.error(f"Device chart failed: {str(e)[:200]}")

# 8. Fraud by Location
if "location" in display_df.columns and "fraud_probability" in display_df.columns:
    st.markdown("#### 🌍 Fraud by Location")
    try:
        loc = display_df.groupby("location")["fraud_probability"].mean().reset_index()
        fig_loc = px.bar(loc, x="location", y="fraud_probability", title="Average Fraud Probability by Location")
        fig_loc.update_layout(height=300)
        st.plotly_chart(fig_loc, use_container_width=True)
    except Exception as e:
        st.error(f"Location chart failed: {str(e)[:200]}")

# 9. Fraud by Channel
if "channel" in display_df.columns and "fraud_probability" in display_df.columns:
    st.markdown("#### 🔗 Fraud by Channel")
    try:
        ch = display_df.groupby("channel")["fraud_probability"].mean().reset_index()
        fig_ch = px.bar(ch, x="channel", y="fraud_probability", title="Average Fraud Probability by Channel")
        fig_ch.update_layout(height=300)
        st.plotly_chart(fig_ch, use_container_width=True)
    except Exception as e:
        st.error(f"Channel chart failed: {str(e)[:200]}")
